...
class ScheduleDateController extends Controller
{
    ...
    public function download(ScheduleDate $scheduleDate)
    {
        ...
        $clinics = Clinic::where('cl_active', true)->orderBy('cl_name')->get();
        ...
        foreach ($clinics as $clinic) {
            try {
                ...
                $response = $client->get("https://mobilejkn.rscahyakawaluyan.com/medinfrasAPI/{$link}/api/physician/available/{$schedule_date}/{$clinic->cl_code}", [
                    'headers' => $headers,
                ]);
                        ...
                        for ($x = 0; $x < count($dataField); $x++) {
                            for ($y = 1; $y <= 5; $y++) {
                                ...
                                $sc_id_temp = Schedule::create([
                                    ...
                                    'sc_clinic_department' => $clinic->cl_department,
                                    'sc_clinic_visit_type' => $clinic->cl_visit_type,
