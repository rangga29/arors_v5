...
class SCOldAppointment extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.old-sunday-clinic.osc-appointment')
            ->layout('frontend.layout', [
                'subTitle' => 'Form Pasien Sunday Clinic Umum / Asuransi / Kontraktor',
                'description' => 'Form Registrasi Online Rumah Sakit Cahya Kawaluyan untuk Pasien Sunday Clinic Umum / Asuransi / Kontraktor',
    ...
    public function updatedSelectedDate(): void
    {
        if ($this->selectedDate) {
            ...
            $this->clinics = Clinic::whereHas('schedules', function ($query) use ($isToday) {
                ...
                ->where('cl_department', 'OUTPATIENT')
                ->whereNotIn('cl_code', ['RHBKAM001', 'KLI013', 'KLI030', 'REH001'])
    ...
    public function createAppointment()
    {
        ...
        if ($scheduleData['sc_available'] == 0 || $scheduleDetailData['scd_available'] == 0) {
            return redirect()->route('sunday-clinic.old-patient')->with('error', 'Jadwal [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Tidak Tersedia');
        }
        ...
        if ($scheduleDetailData['scd_counter_online_umum'] === $scheduleDetailData['scd_online_umum']) {
            if ($scheduleDetailData['scd_counter_online_bpjs'] === $scheduleDetailData['scd_online_bpjs']) {
                return redirect()->route('sunday-clinic.old-patient')->with('error', 'Kuota Online Pasien Umum / Asuransi [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Sudah Terpenuhi. Silahkan Datang Langsung Ke Rumah Sakit Untuk Pendaftaran Langsung.');
            }
        }
        ...
        if ($this->serviceType == 'umum') {
            ...
            try {
                ...
                if ($response->getStatusCode() == 200) {
                    ...
                    return redirect()->route('sunday-clinic.old-patient')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [' . $response->getStatusCode() . ']');
                }
            } catch (RequestException $e) {
                return redirect()->route('sunday-clinic.old-patient')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [500]');
            }
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if ($data['Status'] == 'SUCCESS (000)') {
                    ...
                    if ($this->serviceType == 'umum') {
                        ...
                        return redirect()->route('sunday-clinic.old-patient.umum.final', $dataField['AppointmentID'])->with('success', 'Registrasi Berhasil Dilakukan');
                    } else {
                        $appointmentData = \App\Models\Appointment::create([
                            'scd_id' => $scheduleDetailData['id'],
                            'ap_ucode' => $dataField['AppointmentID'],
                        ...
                        return redirect()->route('sunday-clinic.old-patient.asuransi.final', $dataField['AppointmentID'])->with('success', 'Registrasi Berhasil Dilakukan');
                    }
                } else {
                    if ($data['Status'] == 'FAILED (102)') {
                        ...
                        if ($this->serviceType == 'umum') {
                            ...
                            $existingAppointment = \App\Models\AsuransiAppointment::with('appointment.scheduleDetail.schedule.scheduleDate')
                        ...
                        if ($existingAppointment) {
                            ...
                            return redirect()->route('sunday-clinic.old-patient')->with('error1', $data['Remarks']);
                        }
                    } else {
