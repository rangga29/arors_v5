...
{
    public $patientData, $serviceType, $address, $phone_number, $email;
    ...
    public function render()
    {
        ...
        return view('livewire.new-sunday-clinic.nsc-appointment')
            ->layout('frontend.layout', [
                ...
                'subKeywords' => 'form pasien klinik minggu, form pasien, pasien klinik minggu'
            ]);
    }
    ...
    public function createAppointment()
    {
        ...
        if ($scheduleData['sc_available'] == 0 || $scheduleDetailData['scd_available'] == 0) {
            return redirect()->route('sunday-clinic.new-patient')->with('error', 'Jadwal [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Tidak Tersedia');
        }
        ...
        if ($scheduleDetailData['scd_counter_online_umum'] === $scheduleDetailData['scd_online_umum']) {
            if ($scheduleDetailData['scd_counter_online_bpjs'] === $scheduleDetailData['scd_online_bpjs']) {
                return redirect()->route('sunday-clinic.new-patient')->with('error', 'Kuota Online Pasien Umum / Asuransi [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Sudah Terpenuhi. Silahkan Datang Langsung Ke Rumah Sakit Untuk Pendaftaran Langsung.');
            }
        }
        ...
        if ($this->serviceType == 'umum') {
            ...
            try {
                ...
                if ($response->getStatusCode() == 200) {
                    ...
                    return redirect()->route('sunday-clinic.new-patient')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [' . $response->getStatusCode() . ']');
                }
            } catch (RequestException $e) {
                return redirect()->route('sunday-clinic.new-patient')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [500]');
            }
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if ($data['Status'] == 'SUCCESS (000)') {
                    ...
                    NewAppointment::create([
                        ...
                        'nap_business_partner_code' => $this->serviceType == 'umum' ? $requestData['BusinessPartnerCode'] : $businessPartnerData['bp_code'],
                        'nap_business_partner_name' => $this->serviceType == 'umum' ? $requestData['BusinessPartnerCode'] : $businessPartnerData['bp_name']
