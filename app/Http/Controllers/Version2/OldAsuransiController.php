...
class OldAsuransiController extends Controller
{
    ...
    {
        return view('version2.asuransi-1', [
            'background' => 'umum',
    ...
    public function patientCheck(Request $request)
    {
        if(!$this->appointmentOpen->selectAppointmentOpen()) {
            return redirect()->route('old-asuransi');
        }
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if (isset($data['Data'])) {
                    ...
                    if ($birthdate == $dataField['DateOfBirth']) {
                        ...
                        return redirect()->route('old-asuransi.appointment-create', $patient_temp['pt_ucode']);
                    } else {
    ...
    public function appointmentCreate(PatientTemporary $patientTemporary)
    {
        ...
        return view('version2.asuransi-2', [
            'background' => 'umum',
            ...
            'businessPartners' => $sortedBusinessPartners
        ]);
    }
    ...
    public function appointmentStore(Request $request, PatientTemporary $patientTemporary)
    {
        ...
        if($scheduleData['sc_available'] == 0 || $scheduleDetailData['scd_available'] == 0) {
            return redirect()->route('old-asuransi')->with('error', 'Jadwal [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Tidak Tersedia');
        }
        ...
        if($scheduleDetailData['scd_counter_online_umum'] === $scheduleDetailData['scd_online_umum']) {
            if($scheduleDetailData['scd_counter_online_bpjs'] === $scheduleDetailData['scd_online_bpjs']) {
                return redirect()->route('old-asuransi')->with('error', 'Kuota Online Pasien Umum / Asuransi [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Sudah Terpenuhi. Silahkan Datang Langsung Ke Rumah Sakit Untuk Pendaftaran Langsung.');
            }
        }
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if (!empty($data['Data'])) {
                    ...
                    $businessPartners = [];
                    for ($x = 0; $x < count($dataField); $x++) {
                        if($dataField[$x]['BusinessPartnerCode'] == $request->selectedBusinessPartner) {
                            $businessPartnerData = [
                                'bp_code' => $dataField[$x]['BusinessPartnerCode'],
                                'bp_name' => $dataField[$x]['BusinessPartnerName'],
                                'bp_contract' => $dataField[$x]['ContractInfo'][0]['ContractNo']
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if($data['Status'] == 'SUCCESS (000)') {
                    ...
                    AsuransiAppointment::create([
                        ...
                        'aap_birthday' => $patientTemporary['pt_birthday'],
                        'aap_gender' => $patientTemporary['pt_gender'],
                        'aap_phone' => $request->phone_number,
                    ...
                    return redirect()->route('old-asuransi')->with('error', $data['Status'] . ' - ' . $data['Remarks']);
                }
            } else {
                return back()->with('error', 'Request failed. Status code: ' . $response->getStatusCode());
            }
