<main class="background">
    <div class="receipt-card">
        <div class="receipt-header">
            ...
            <h2>REGISTRASI PASIEN BARU SUNDAY CLINIC
                {{ $appointmentDetailData['nap_business_partner_code'] == 'PERSONAL' ? 'UMUM' : 'ASURANSI' }}</h2>
        ...
        <div class="data-row">
            ...
            <span class="data-value">{{ $appointmentDetailData['nap_ssn'] }}</span>
        </div>
        <div class="data-row">
            <span class="data-label">Nama Pasien</span>
            <span class="data-value">{{ $appointmentDetailData['nap_name'] }}</span>
        </div>
        <div class="data-row">
            <span class="data-label">Tanggal Pendaftaran</span>
            <span
                class="data-value">{{ \Carbon\Carbon::createFromFormat('Y-m-d', $scheduleDateData['sd_date'])->isoFormat('dddd, DD MMMM YYYY') }}</span>
        </div>
        <div class="data-row">
            <span class="data-label">Nama Klinik</span>
            <span class="data-value">{{ $scheduleData['sc_clinic_name'] }}</span>
        </div>
        <div class="data-row">
            <span class="data-label">Nama Dokter</span>
            <span class="data-value">{{ $scheduleData['sc_doctor_name'] }}</span>
        </div>
        <div class="data-row">
        ...
        @if ($appointmentDetailData['nap_business_partner_code'] != 'PERSONAL')
            <div class="data-row">
                ...
                <span class="data-value">{{ $appointmentDetailData['nap_business_partner_name'] }}</span>
            </div>
        @endif
        <div class="data-row">
        ...
        <div class="receipt-actions">
            <button class="w-100 btn btn-primary text-uppercase mb-2" wire:click="downloadPdf"
                wire:loading.attr="disabled">
                <i class="ri-download-2-line"></i> Download Bukti Pendaftaran
            </button>
            <a href="{{ route('home') }}" class="w-100 btn btn-danger text-uppercase">
                <i class="ri-home-4-line"></i> Kembali Ke Halaman Utama
            </a>
        </div>
    </div>
</main>
