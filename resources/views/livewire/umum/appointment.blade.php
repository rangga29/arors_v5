<div>
    ...
    <div class="reg-announcement reg-announcement-warning">
        <div class="reg-announcement-content">
            Anda Terdaftar Sebagai <strong>PASIEN NON BPJS</strong> dan Nomor Antrian <strong>TIDAK DAPAT</strong>
            Digunakan Untuk Mendaftar Sebagai Pasien BPJS <strong>KECUALI</strong> Pasien Klinik Rehabilitasi Medik
        </div>
    </div>
    ...
    <form wire:submit.prevent="createAppointment">
        ...
        <div class="reg-info-row">
            ...
            <span class="reg-info-value">{{ $patientData['FullName'] }}</span>
        </div>
        <div class="reg-info-row" style="margin-bottom: 1.25rem;">
            <span class="reg-info-label">Tanggal Lahir</span>
            <span
                class="reg-info-value">{{ \Carbon\Carbon::createFromFormat('Ymd', $patientData['DateOfBirth'])->isoFormat('DD MMMM YYYY') }}</span>
        </div>
        ...
        <div class="reg-form-group">
            ...
            <input type="text" class="reg-input" name="phone_number" id="phone_number" wire:model="phone_number"
                placeholder="Masukkan No Handphone" oninput="this.value = this.value.replace(/\D/g, '');" autofocus
        ...
        @if ($serviceType == 'asuransi')
            <div class="reg-form-group">
                <label for="selectedBusinessPartner" class="reg-label">Asuransi / Kontraktor</label>
                <select class="reg-select" id="selectedBusinessPartner" wire:model="selectedBusinessPartner" required>
                    <option value="">Pilih Asuransi / Kontraktor</option>
