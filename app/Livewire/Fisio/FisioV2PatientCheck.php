...
class FisioV2PatientCheck extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.fisio.fisio-v2-patient-check', [
    ...
    public function updatedSelectedDate(): void
    {
        ...
        if ($this->clinics instanceof \Illuminate\Support\Collection && $this->clinics->count() === 1) {
            $this->selectedClinic = $this->clinics->first()->cl_code;
            $this->updatedSelectedClinic();
        }
    }
    ...
    public function updatedSelectedClinic(): void
    {
        ...
        if ($this->doctors instanceof \Illuminate\Support\Collection && $this->doctors->count() === 1) {
            $this->selectedDoctor = $this->doctors->first()->sc_doctor_code;
            $this->updatedSelectedDoctor();
        }
    }
    ...
    public function updatedSelectedDoctor(): void
    {
        $selectedDateObject = $this->dates->firstWhere('id', $this->selectedDate);
        $isToday = $selectedDateObject && \Carbon\Carbon::parse($selectedDateObject->sd_date)->isToday();
        $isSaturday = $selectedDateObject && \Carbon\Carbon::parse($selectedDateObject->sd_date)->isSaturday();
        $timeBoundary = $isSaturday ? '12:00:00' : '14:00:00';
        ...
        if ($this->patientStatus == 'UMUM PAGI' || $this->patientStatus == 'ASURANSI PAGI') {
            $query = ScheduleDetail::whereHas('schedule', function ($query) {
                ...
                ->where('scd_start_time', '<', $timeBoundary)
                ->where('scd_umum', true)
                ->where('scd_available', true);
        ...
        if ($this->sessions instanceof \Illuminate\Support\Collection && $this->sessions->count() === 1) {
    ...
    public function createAppointmentUmum()
    {
        ...
        if ($scheduleData['sc_available'] == 0 || $scheduleDetailData['scd_available'] == 0) {
            return redirect()->route('fisioterapi')->with('error', 'Mohon Maaf Jadwal [' . $scheduleData['sc_clinic_name'] . ' -- ' . $scheduleData['sc_doctor_name'] . '] Tidak Tersedia');
        }
                        ...
                        try {
                            ...
                            if ($responseApp->getStatusCode() == 200) {
                                ...
                                if ($dataApp['Status'] == 'SUCCESS (000)') {
                                    ...
                                    return redirect()->route('fisioterapi.umum.final', $dataFieldApp['AppointmentID'])->with('success', 'Registrasi Berhasil Dilakukan');
                                } else {
                                    return redirect()->route('fisioterapi')->with('error', $dataApp['Status'] . ' - ' . $dataApp['Remarks']);
                                }
                            } else {
                                return redirect()->route('fisioterapi')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [' . $responseApp->getStatusCode() . ']');
                            }
                        } catch (RequestException $e) {
                            return redirect()->route('fisioterapi')->with('error', 'Mohon Maaf Terjadi Kesalahan Pada Sistem. Silahkan Menghubungi Customer Service di 0812 1111 8009. Terima Kasih. [500] - 2');
                        }
                    } else {
                        ...
                        try {
                            ...
                            if ($responseApp->getStatusCode() == 200) {
                                ...
                                if ($dataApp['Status'] == 'SUCCESS (000)') {
                                    ...
                                    return redirect()->route('fisioterapi.asuransi.final', $dataFieldApp['AppointmentID'])->with('success', 'Registrasi Berhasil Dilakukan');
                                } else {
                                    return redirect()->route('fisioterapi')->with('error', $dataApp['Status'] . ' - ' . $dataApp['Remarks']);
                                }
                            } else {
                        ...
                        try {
                            ...
                            if ($responseApp->getStatusCode() == 200) {
                                ...
                                if ($dataApp['Status'] == 'SUCCESS (000)') {
                                    ...
                                    return redirect()->route('fisioterapi.bpjs.final', $dataFieldApp['AppointmentID'])->with('success', 'Registrasi Berhasil Dilakukan');
                                } else {
                                    return redirect()->route('fisioterapi')->with('error', $dataApp['Status'] . ' - ' . $dataApp['Remarks']);
                                }
                            } else {
